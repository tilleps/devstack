...
/**
 * @typedef {Array<[Error, string, string, number, boolean]>} HealthCheckResult2
 */
/**
...
class HealthCheck {
  ...
  async isOperational(healthCheckResults) {
    ...
    const notUpCount = healthCheckResults.reduce(function (count, HealthCheckResult) {
      const { state, optional } = HealthCheckResult;

      if (!optional && state !== "up") {
        count++;
      }
    ...
    for (const dependency of this._dependencies) {
      ...
      const healthCheck = new Promise(async function (resolve, reject) {
        ...
        const timer = setTimeout(function () {
          resolve([null, name, "timeout", calculateDurationMilliseconds(start), optional]);
        }, 10000);

        try {
          let result = await dependency.health?.();
          if (!!result) {
            resolve([null, name, "up", calculateDurationMilliseconds(start), optional]);
          } else {
            resolve([null, name, "down", calculateDurationMilliseconds(start), optional]);
          }
        } catch (err) {
          resolve([err, name, "error", calculateDurationMilliseconds(start), optional]);
        }
        clearTimeout(timer);
      });
